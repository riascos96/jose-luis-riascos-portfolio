  ...
  useGSAP(
    () => {
      ...
      mm.add("(prefers-reduced-motion: reduce)", () => {
        gsap.set(
          "[data-timeline-progress], [data-experience-card], [data-experience-shell], [data-experience-date], [data-experience-marker], [data-experience-bullet]",
          { clearProps: "all" }
        );
      });

      mm.add("(min-width: 1024px) and (prefers-reduced-motion: no-preference)", () => {
        createDesktopTrajectory();
      });

      mm.add("(max-width: 1023px) and (prefers-reduced-motion: no-preference)", () => {
        createMobileTrajectory();
      });
                  ...
                  <div className="relative grid gap-6 lg:grid-cols-[3rem_minmax(0,1fr)] lg:gap-8">
                    ...
                    <div
                      ...
                      className={`relative origin-left rounded-4xl border bg-[linear-gradient(180deg,rgba(255,255,255,0.99),rgba(248,250,252,0.92))] p-6 shadow-[0_24px_64px_rgba(27,59,90,0.08)] sm:p-7 lg:p-8 ${
                        isCurrent ? "border-[rgba(0,119,182,0.26)]" : "border-(--line)"
                      }`}
                    >
